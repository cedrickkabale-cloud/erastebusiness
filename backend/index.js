...
app.get('/api/invoices/:id/pdf/preview', (req, res) => {
  ...
  db.get('SELECT * FROM invoices WHERE id = ?', [id], (err, invoice) => {
    ...
    db.all('SELECT * FROM invoice_lines WHERE invoice_id = ? ORDER BY numero_ordre', [id], async (err2, lines) => {
      ...
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `inline; filename="${invoice.numero_facture}-preview.pdf"`);
      // create A4 document with margins
      const doc = new PDFDocument({ size: 'A4', margin: 40 });
      doc.pipe(res);

      // draw logo if exists (expected path: backend/public/logo-eraste.png)
      const logoPath = path.join(__dirname, 'public', 'logo-eraste.png');
      try{
        const fs = require('fs');
        if (fs.existsSync(logoPath)){
      ...
      doc.fontSize(16).text('Ets Eraste Business SARL', { align: 'center' });
      doc.moveDown(0.2);
      doc.fontSize(12).text('Marché MITENDI', { align: 'center' });
      doc.moveDown(0.5);

      // header info
      const leftX = doc.page.margins.left;
      const rightX = doc.page.width - doc.page.margins.right;
      doc.fontSize(10).text(`Facture N°: ${invoice.numero_facture}`, leftX, doc.y + 10);
      doc.text(`Date: ${invoice.date_emission}`, rightX - 200, doc.y - 12);
      doc.moveDown(0.5);
      doc.text(`Heure: ${invoice.heure_emission}`);
      doc.text(`Client: ${invoice.nom_client}`);
      doc.moveDown(0.8);
      ...
      const itemX = leftX;
      const qtyX = leftX + 250;
      const priceX = leftX + 320;
      const amountX = rightX - 80;

      doc.fontSize(10).text('N°', itemX, tableTop, { bold: true });
      doc.text('Designation', itemX + 20, tableTop);
      doc.text('Qte', qtyX, tableTop);
      doc.text('Prix U', priceX, tableTop);
      ...
      let y = doc.y + 8;
      lines.forEach(l => {
        doc.fontSize(10).text(l.numero_ordre.toString(), itemX, y);
        doc.text(l.designation, itemX + 20, y, { width: 220 });
        doc.text(Number(l.quantite).toString(), qtyX, y);
        doc.text(Number(l.prix_unitaire).toFixed(2), priceX, y);
        doc.text(Number(l.montant).toFixed(2), amountX, y);
        y += 18;
      ...
      doc.fontSize(12).text(`Total: ${Number(invoice.total).toFixed(2)} ${invoice.devise}`, amountX, y + 8);
      ...
      try{
        ...
        const base64 = dataUrl.split(',')[1];
        const img = Buffer.from(base64, 'base64');
        doc.image(img, doc.page.width/2 - 40, doc.page.height - 160, { width: 80 });
      }catch(e){ }

      doc.fontSize(9).text("La marchandise vendue n'est ni reprise ni échangée", leftX, doc.page.height - 80, { align: 'center', width: doc.page.width - 80 });
      doc.fontSize(9).text('Merci et à la prochaine', { align: 'center' });
      doc.end();
    });
  });
});
